...
def _safe_value(value: Any, depth: int = 0) -> Any:
    ...
    if isinstance(value, dict):
        blocked = {
            "api_key", "apikey", "authorization", "headers", "secret", "token",
            "access_token", "auth_token", "signature", "password", "private_key",
        }
...
def _resource_model(kwargs: Mapping[str, Any]) -> Any:
    """Read a model only from a named parameter on non-Message resources."""
    return kwargs.get("model")


_REQUEST_FIELDS = (
    "messages", "system", "tools", "thinking", "tool_choice", "output_config",
    "output_format", "cache_control", "context_management", "mcp_servers",
    "fallbacks", "fallback_credit_token", "speed", "inference_geo", "container",
    "metadata", "service_tier", "stop_sequences", "betas", "user_profile_id",
)
...
def _request_payload(args: Tuple[Any, ...], kwargs: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    if messages is None and len(args) >= 2:
        messages = args[1]
    payload = {key: kwargs.get(key) for key in _REQUEST_FIELDS if kwargs.get(key) is not None}
...
def _usage(resp: Any) -> Dict[str, Any]:
    ...
    for name in ("input_tokens", "output_tokens", "cache_creation_input_tokens",
                 "cache_read_input_tokens", "thinking_tokens", "cache_read"):
        value = _field(usage, name)
        if isinstance(value, (str, int, float, bool)):
            result[name] = value
    # Newer SDK versions expose cache and server-tool counters as nested
    # objects. Flatten them into primitive attributes accepted by OpenTelemetry.
    for parent, fields in (
        ("cache_creation", ("ephemeral_5m_input_tokens", "ephemeral_1h_input_tokens")),
        ("server_tool_use", ("web_search_requests", "web_fetch_requests")),
    ):
        nested = _field(usage, parent)
        for name in fields:
    ...
    if isinstance(thinking, (str, int, float, bool)):
...
class _StreamTraceState:
    ...
    def observe_event(self, event: Any) -> None:
        ...
        if event_type == "message_delta":
            ...
            if _field(block, "type") in {"tool_use", "server_tool_use"}:
            ...
            if delta_type == "tool_change":
                self._completion_parts.append("[tool_change]")
...
class _StreamProxy:
    ...
    def _finish(
        ...
        for event_type, count in result.event_counts.items():
            self._span.set_attribute("llm.anthropic.stream." + event_type + ".count", count)
        _close_stream_span(
            self._span,
            self._started,
            self._request_model,
...
def _open_resource_stream_span(
    resource: str,
    model: Any,
    args: Tuple[Any, ...],
    kwargs: Mapping[str, Any],
    ...
    tracer = _get_tracer("anthropic")
    attrs = _resource_attrs(resource, "stream", model, args, kwargs)
    span_name = "llm.anthropic." + resource + ".stream"
    started = time.perf_counter()
    if hasattr(tracer, "start_span"):
        span = tracer.start_span(span_name, attributes=attrs)
        span.__enter__()
        return span, started, None
    context_manager = tracer.start_as_current_span(span_name, attributes=attrs)
    span = context_manager.__enter__()
    return span, started, context_manager.__exit__
...
def _trace_call(
    ...
    args: Tuple[Any, ...],
    kwargs: Mapping[str, Any],
    span_name: str = "llm.anthropic.messages",
    ...
    with tracer.start_as_current_span(
        span_name,
        attributes=dict(attributes or _start_attrs(model, args, kwargs)),
    ) as span:
        try:
            yield span, started
        except BaseException as exc:
...
def _resource_attrs(
    ...
    """Build bounded attributes for non-Message Anthropic resources."""
    attrs: Dict[str, Any] = {
        "llm.vendor": "anthropic",
        "gen_ai.system": "anthropic",
        "gen_ai.operation.name": (
            "invoke_agent"
            if (
                resource.startswith("beta.agents")
                or resource.startswith("beta.deployments")
                or resource.startswith("beta.sessions")
            )
            else "chat"
        ),
        "anthropic.resource": resource,
        "anthropic.operation": operation,
    }
    if model:
        attrs["llm.model"] = model
        attrs["gen_ai.request.model"] = model
    # Webhook verification helpers receive the signing secret positionally;
    # never attach positional values for those operations.
    if operation not in {"unwrap", "parse_unverified"}:
        for index, value in enumerate(args):
            encoded = _json_attr(value)
            if encoded:
                attrs["anthropic.request.arg" + str(index)] = encoded
    for key, value in kwargs.items():
        if key in {
            "extra_headers", "extra_query", "extra_body", "timeout", "secret",
            "api_key", "api_secret", "token", "access_token", "auth_token",
            "signature", "password", "private_key",
        }:
            continue
        encoded = _json_attr(value)
        if encoded:
            attrs["anthropic.request." + str(key)] = encoded
    return attrs
...
def _call_operation(
    ...
    model = _resource_model(kwargs)
    with _trace_call(
        model,
        args,
        kwargs,
        span_name="llm.anthropic." + resource + "." + operation,
        attributes=_resource_attrs(resource, operation, model, args, kwargs),
    ) as (span, started):
        response = fn(self, *args, **kwargs)
        _finish(span, response, model, started)
        return response
...
def _wrap_operation(
    ...
    if is_async:
        ...
        async def wrapped_async(self: Any, *args: Any, **kwargs: Any) -> Any:
            return await _call_operation_async(fn, self, args, kwargs, resource, operation)
        wrapped = wrapped_async
    else:
        @functools.wraps(fn)
        def wrapped_sync(self: Any, *args: Any, **kwargs: Any) -> Any:
            return _call_operation(fn, self, args, kwargs, resource, operation)
...
def _wrap_resource_stream(fn: Callable[..., Any], is_async: bool, resource: str) -> Callable[..., Any]:
    if getattr(fn, "_agent_trace_patched", False):
        return fn

    if is_async:
        @functools.wraps(fn)
        async def wrapped_async(self: Any, *args: Any, **kwargs: Any) -> _StreamProxy:
            model = _resource_model(kwargs)
            span_state = _open_resource_stream_span(resource, model, args, kwargs)
            try:
                stream = await fn(self, *args, **kwargs)
            except BaseException as exc:
                span, started, span_exit = span_state
...
def _public_operations(resource_cls: Any, excluded: Tuple[str, ...] = ()) -> Tuple[str, ...]:
    """Return concrete public endpoint methods defined by an SDK resource.

    Anthropic's generated SDK adds resources and operations frequently. Static
    inspection lets us cover new endpoints without maintaining a brittle list,
    while excluding cached child resources and response wrapper descriptors.
    """
    operations = []
    getmembers = getattr(inspect, "getmembers_static", inspect.getmembers)
    for name, member in getmembers(resource_cls):
        if name.startswith("_") or name in excluded:
            continue
        if isinstance(member, (property, functools.cached_property, staticmethod, classmethod)):
            continue
        if callable(member):
...
def _patch_resource_class(
    resource_cls: Any,
    is_async: bool,
    resource: str = "messages",
    operations: Optional[Tuple[str, ...]] = None,
) -> bool:
    """Patch one SDK resource class while tolerating SDK version differences."""
    changed = False
    if resource == "messages":
        create = getattr(resource_cls, "create", None)
        if create is not None and not getattr(create, "_agent_trace_patched", False):
            resource_cls.create = _wrap_create(create, is_async)
            changed = True
        stream = getattr(resource_cls, "stream", None)
        if stream is not None and not getattr(stream, "_agent_trace_patched", False):
            resource_cls.stream = _wrap_stream(stream)
            changed = True
        operations = operations or _public_operations(resource_cls, ("create", "stream"))
    else:
        operations = operations or _public_operations(resource_cls)

    for operation in operations:
        fn = getattr(resource_cls, operation, None)
        if fn is not None and not getattr(fn, "_agent_trace_patched", False):
            # Generated Async SDK resources mix coroutine methods with regular
            # pagination/helper methods. Only await functions that are actually
            # coroutine functions so async iterators and paginators keep their
            # original return protocol.
            method_is_async = is_async and inspect.iscoroutinefunction(fn)
            if operation == "stream" and resource != "messages":
                setattr(resource_cls, operation, _wrap_resource_stream(fn, method_is_async, resource))
                changed = True
                continue
            setattr(resource_cls, operation, _wrap_operation(fn, method_is_async, resource, operation))
            changed = True
    return changed or any(getattr(resource_cls, operation, None) is not None for operation in operations)


def patch_anthropic() -> bool:
    """Patch Anthropic SDK resources for sync, async, and streaming calls.

    Resource paths are intentionally imported lazily and independently so the
    integration continues to work across Anthropic SDK versions where beta
    resources move or are not installed yet.
    """
    global _patched
    if _patched:
        return True
    ...
    patched = False
    module_specs = (
        ("anthropic.resources.messages.messages", "messages", ("Messages", "AsyncMessages")),
        ("anthropic.resources.beta.messages.messages", "messages", ("Messages", "AsyncMessages")),
        ("anthropic.resources.messages.batches", "messages.batches", ("Batches", "AsyncBatches")),
        ("anthropic.resources.beta.messages.batches", "messages.batches", ("Batches", "AsyncBatches")),
        ("anthropic.resources.beta.agents.agents", "beta.agents", ("Agents", "AsyncAgents")),
        ("anthropic.resources.beta.agents.versions", "beta.agents.versions", ("Versions", "AsyncVersions")),
        ("anthropic.resources.beta.deployments", "beta.deployments", ("Deployments", "AsyncDeployments")),
        ("anthropic.resources.beta.deployment_runs", "beta.deployment_runs", ("DeploymentRuns", "AsyncDeploymentRuns")),
        ("anthropic.resources.beta.sessions.sessions", "beta.sessions", ("Sessions", "AsyncSessions")),
        ("anthropic.resources.beta.sessions.events", "beta.sessions.events", ("Events", "AsyncEvents")),
        ("anthropic.resources.beta.sessions.resources", "beta.sessions.resources", ("Resources", "AsyncResources")),
        ("anthropic.resources.beta.sessions.threads.threads", "beta.sessions.threads", ("Threads", "AsyncThreads")),
        ("anthropic.resources.beta.sessions.threads.events", "beta.sessions.threads.events", ("Events", "AsyncEvents")),
        ("anthropic.resources.skills.skills", "skills", ("Skills", "AsyncSkills")),
        ("anthropic.resources.skills.versions", "skills.versions", ("Versions", "AsyncVersions")),
        ("anthropic.resources.beta.skills.skills", "beta.skills", ("Skills", "AsyncSkills")),
        ("anthropic.resources.beta.skills.versions", "beta.skills.versions", ("Versions", "AsyncVersions")),
        ("anthropic.resources.files", "files", ("Files", "AsyncFiles")),
        ("anthropic.resources.models", "models", ("Models", "AsyncModels")),
        ("anthropic.resources.beta.files", "beta.files", ("Files", "AsyncFiles")),
        ("anthropic.resources.beta.models", "beta.models", ("Models", "AsyncModels")),
    )
    for module_name, resource, class_names in module_specs:
        try:
            module = importlib.import_module(module_name)
            for class_name, is_async in ((class_names[0], False), (class_names[1], True)):
                resource_cls = getattr(module, class_name, None)
                if resource_cls is not None:
                    patched = _patch_resource_class(resource_cls, is_async, resource) or patched
        except (ImportError, AttributeError):
            continue

    # Cover newly generated resources and operations in future SDK releases.
    # Explicit paths above preserve compatibility with older SDKs and fake SDKs
    # used by downstream tests; discovery fills in the rest when resources are
    # exposed as a normal Python package.
    try:
        resources_pkg = importlib.import_module("anthropic.resources")
        for module_info in pkgutil.walk_packages(
            getattr(resources_pkg, "__path__", ()), resources_pkg.__name__ + "."
        ):
            ...
            try:
                ...
                continue
            suffix = module_name.split("anthropic.resources.", 1)[-1]
            parts = suffix.split(".")
            if len(parts) > 1 and parts[-1] == parts[-2]:
                parts.pop()
            resource = "messages" if suffix.endswith("messages.messages") else ".".join(parts)
            for resource_cls in tuple(
                cls for cls in vars(module).values()
                if isinstance(cls, type) and cls.__module__ == module_name
            ):
                class_name = resource_cls.__name__
                if class_name.endswith(("WithRawResponse", "WithStreamingResponse")):
                    continue
