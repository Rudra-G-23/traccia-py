...
def test_structured_parse_is_patched_for_sync_and_async(monkeypatch):
    class Messages:
        def parse(self, **kwargs):
            return {"id": "parsed", "model": kwargs["model"], "usage": {"input_tokens": 1, "output_tokens": 2}}

    class AsyncMessages:
        async def parse(self, **kwargs):
            return {"id": "parsed-async", "model": kwargs["model"], "usage": {"input_tokens": 3, "output_tokens": 4}}

    tracer = FakeTracer()
    monkeypatch.setattr(anthropic_mod, "_get_tracer", lambda name: tracer)
    assert anthropic_mod._patch_resource_class(Messages, False, "messages")
    assert anthropic_mod._patch_resource_class(AsyncMessages, True, "messages")

    response = Messages().parse(model="claude-parse")
    async_response = asyncio.run(AsyncMessages().parse(model="claude-parse-async"))
    assert response["id"] == "parsed"
    assert async_response["id"] == "parsed-async"
    assert [span.name for span in tracer.spans] == [
        "llm.anthropic.messages.parse",
        "llm.anthropic.messages.parse",
    ]


def test_managed_agents_and_deployment_methods_are_patched(monkeypatch):
    class Agents:
        def create(self, **kwargs):
            return {"id": "agent_123", "model": kwargs["model"]}

    class Deployments:
        def run(self, deployment_id, **kwargs):
            return {"id": "run_123", "deployment_id": deployment_id}

    tracer = FakeTracer()
    monkeypatch.setattr(anthropic_mod, "_get_tracer", lambda name: tracer)
    anthropic_mod._patch_resource_class(Agents, False, "beta.agents")
    anthropic_mod._patch_resource_class(Deployments, False, "beta.deployments")

    agent = Agents().create(model="claude-agent", name="support")
    run = Deployments().run("dep_123")
    assert agent["id"] == "agent_123"
    assert run["id"] == "run_123"
    assert [span.name for span in tracer.spans] == [
        "llm.anthropic.beta.agents.create",
        "llm.anthropic.beta.deployments.run",
    ]
    assert "llm.model" not in tracer.spans[1].attributes
    assert tracer.spans[1].attributes["anthropic.request.arg0"] == '"dep_123"'


def test_async_pagination_helpers_keep_non_awaitable_sdk_protocol(monkeypatch):
    class AsyncAgents:
    ...
    tracer = FakeTracer()
    monkeypatch.setattr(anthropic_mod, "_get_tracer", lambda name: tracer)
    anthropic_mod._patch_resource_class(AsyncAgents, True, "beta.agents")

    page = AsyncAgents().list()
    retrieved = asyncio.run(AsyncAgents().retrieve("agent_123"))
    assert page is not None
    assert retrieved["id"] == "agent_123"
    assert [span.name for span in tracer.spans] == [
        "llm.anthropic.beta.agents.list",
        "llm.anthropic.beta.agents.retrieve",
    ]


def test_new_usage_counters_are_flattened_and_tool_change_is_counted():
    usage = types.SimpleNamespace(
        input_tokens=10,
        output_tokens=4,
        cache_creation=types.SimpleNamespace(ephemeral_5m_input_tokens=2, ephemeral_1h_input_tokens=3),
        server_tool_use=types.SimpleNamespace(web_search_requests=1, web_fetch_requests=2),
    )
    assert anthropic_mod._usage(types.SimpleNamespace(usage=usage)) == {
        "input_tokens": 10,
        "output_tokens": 4,
    ...
    state = anthropic_mod._StreamTraceState()
    state.observe_event({"type": "content_block_delta", "delta": {"type": "tool_change"}})
    result = state.resolve()
    assert result.event_counts["content_block_delta"] == 1
    assert result.completion == "[tool_change]"
